...
description: Before git push, run the full local CI mirror (pnpm ci:local), auto-fix failures, re-run until green, then push only if the user explicitly asked. Use whenever the user requests push, commit-and-push, or publish to GitHub. Also enforced by .githooks/pre-push.
---
...
Also enforced mechanically by **`.githooks/pre-push`** (installed via `pnpm hooks:install` / `prepare`).
...
2. Run **`pnpm ci:local`** (mirrors `.github/workflows/ci.yml`: install → audit:schema → build → typecheck → test → test:rls → playwright chromium → test:e2e).
   - On success, writes `.git/edunudg-ci-local.ok` so the git hook can skip a duplicate run for ~30 minutes on the same HEAD.
3. On **any** failure:
...
- `SKIP_CI_LOCAL=1` on `git push` only for user-approved emergencies (hook bypass).
...
- Hook: `.githooks/pre-push` (install: `pnpm hooks:install`)
