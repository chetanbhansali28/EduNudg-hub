...
Center Leads / Students / Fees match that Curriculum chrome: Open / Converted / Lost / Total (`regression_center_leads_pipeline_workspace_theme`); Linked / Unassigned / Programs / Total (`regression_center_students_page_matches_curriculum_stats_chrome`); Outstanding / Paid / Overdue / Total (`regression_center_fees_page_matches_curriculum_stats_chrome`). Assert **Add invoice** before switching to Payments (that tab hides the add form).
