...
Center Leads / Students / Fees / Inventory / Merchandise match that Curriculum chrome: Open / Converted / Lost / Total (`regression_center_leads_pipeline_workspace_theme`); Linked / Unassigned / Programs / Total (`regression_center_students_page_matches_curriculum_stats_chrome`); Outstanding / Paid / Overdue / Total (`regression_center_fees_page_matches_curriculum_stats_chrome`); In stock / Low stock / Incoming / Total (`regression_center_inventory_page_matches_curriculum_stats_chrome`); item photo in column 2 is left-aligned at 50% beside stock facts (`regression_center_inventory_detail_photo_is_half_width`); On the way and Orders share a row (`regression_center_inventory_detail_splits_on_the_way_and_orders`); column 2 is `PipelineDetailPanel` with extra top padding and a primary **Place New Order** button (`regression_center_inventory_detail_uses_pipeline_theme`); Catalog / Unpaid / Orders / Total (`regression_center_merchandise_page_matches_curriculum_stats_chrome`). Shop catalog cards are horizontal one-per-row (`regression_center_merchandise_shop_cards_are_horizontal_one_per_row`). Desktop Shop list column matches Curriculum width (`regression_center_merchandise_list_column_matches_curriculum_width`). **Add to Order** is a full-width footer under qty so the label is not truncated (`regression_center_merchandise_shop_add_label_is_not_truncated`). Do not show the boilerplate description “Training kits and supplies for your center.” (`regression_center_merchandise_shop_omits_placeholder_description`). Assert **Add invoice** before switching to Payments (that tab hides the add form).
