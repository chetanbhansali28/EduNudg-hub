...
- Student/login chrome: never clear `logo_url` / login copy when patching unrelated settings. Brand `/app/settings` has no Brand Identity / logo upload — Site logo lives on `/app/homepage` Site accordion and syncs to `brands.logo_url` on save (`regression_brand_settings_omits_brand_identity_card`, `regression_save_brand_landing_syncs_site_logo_to_brands_logo_url`).
