...
- EduLearn MUST render Spark/Abacus stored landing (courses from Curriculum, FAQ, **Apply franchise** nav, Mentors / Leadership at `#founders`) — restyle only, do not hide those sections by default. Leadership cards show Role badge + Title (`regression_edu_learn_mentor_card_shows_role_badge_and_title`).
...
- Brand `/app/curriculum` create **and** existing-course editor share parent marketing fields (benefits, why parents choose this, skills & outcomes, scholarship). Do not drop those fields from `CurriculumCourseDetail` after `createProgram` — `regression_created_course_shows_parent_marketing_fields`. Course Banner preview with an uploaded image uses `--filled` (no inner padding, full-width `height: auto`) — `regression_curriculum_banner_preview_fills_frame_without_side_gaps`.
...
- Spark Academy **Courses designed for success** uses published `publicCurriculum` (`resolveSparkCoursePrograms`), not leftover homepage program cards. Cards are fallback only when no published courses exist — `regression_spark_courses_use_published_curriculum_over_homepage_cards`. Published syllabus still renders when leftover `programsGrid` is off (`sparkShouldShowCoursesSection`, `regression_spark_courses_show_published_syllabus_even_if_programs_grid_off`). The section title is center-aligned (`sa-section-head--center`). Course cards center in the row (`sa-courses__grid--center`, `regression_spark_course_cards_center_in_grid`). Course cards keep **Enroll now** only (no **Enroll** price/link); rating is centered below the button — `regression_spark_course_card_keeps_enroll_now_and_centers_rating_below`. Course media and title link to `/courses/:slug` (`regression_spark_course_card_links_to_public_detail`). Course banners use a full-width `img` with `object-fit: contain` (`regression_spark_course_card_banner_shows_full_image`). Spark `/courses/:slug` uses `--sa-page` (`#f8fafc`) like the courses section (`regression_spark_course_page_uses_courses_section_background`) and a sticky enroll offer card (`regression_public_course_page_has_enroll_offer_card`). Do not add **All courses** / course-name filter tabs — `regression_spark_courses_section_has_no_curriculum_tabs`. List every published course in the wrapping grid (`regression_spark_courses_lists_all_published_programs`). Public `/courses/:slug` uses the same layout chrome as `/` (`PublicCoursePage`).
- Spark Academy **Trust & video** YouTube URL renders under Journey (`sa-journey__video`, `#trust`) without replacing the highlight photo — `regression_spark_journey_renders_youtube_below_photo`, `regression_spark_journey_omits_youtube_when_url_empty`. EduLearn shows the same embed under stats (`regression_edu_learn_stats_renders_youtube_below_photo`). Shorts URLs use `toYoutubeEmbedUrl` — `regression_youtube_shorts_url_converts_to_embed`.
...
- Spark Academy **Meet Our Expert Mentors** centers mentor cards in the row (`sa-mentors__track--center`) — `regression_spark_mentors_center_in_track`. Cards show Homepage **Role badge** and **Title** together (`regression_spark_mentor_card_shows_role_badge_and_title`); do not use `title || roleBadge`. Brand `/app/homepage` edits this list in **Mentors / Leadership** (not a section titled Mentors). Template **Founder name** is hidden on the public site (`regression_brand_founders_omit_founder_name_placeholder_even_with_photo`).
