...
- Brand Student Leads / Center Leads: **Add lead** uses the same modal pattern (`ManualStudentLeadCard` + `ed-import-dialog`), not `AddFormSection`. Center `/app/leads`, `/app/students`, and `/app/fees` use Curriculum pipeline chrome (`PipelinePageHeader`, `LeadKpiGrid`, search + `FilterTabs`, `PipelineWorkspace`). Leads KPIs: Open / Converted / Lost / Total. Students: Linked / Unassigned / Programs (informational) / Total. Fees: Outstanding / Paid / Overdue / Total with Invoices / Payments tabs — `regression_center_leads_pipeline_workspace_theme`, `regression_center_students_page_matches_curriculum_stats_chrome`, `regression_center_fees_page_matches_curriculum_stats_chrome`.
