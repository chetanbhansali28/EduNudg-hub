...
Brand **Mentors / Leadership** (homepage accordion; Spark public label is Meet Our Expert Mentors) stores people in `landing.founders`. Template names **Founder name** / **Name** are never shown on the live brand or franchise site (`visiblePublicFounders`). Set the real person’s name in that accordion. Spark mentor cards show **Role badge** and **Title** together (Abacus shows both; Spark used to hide the badge when title was set) — `regression_spark_mentor_card_shows_role_badge_and_title`. EduLearn renders the same fields in **Meet our leadership** (`#founders`) — `regression_edu_learn_mentor_card_shows_role_badge_and_title`.
