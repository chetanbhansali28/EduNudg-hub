...
Each course card keeps **Enroll now** only (no separate **Enroll** price/link). The star rating sits below that button and is centered. Course media and title link to `/courses/:slug` (full `/app/curriculum` marketing record) while **Enroll now** stays a lead-modal button. Spark `/courses/:slug` uses the same `--sa-page` (`#f8fafc`) canvas as **Courses designed for success**, not a plain white sheet (`regression_spark_course_page_uses_courses_section_background`). The section has no **All courses** / course-name filter tabs. Regression: `regression_spark_courses_lists_all_published_programs`, `regression_spark_view_all_courses_hidden_when_catalog_fits`, `regression_spark_course_card_links_to_public_detail`.
