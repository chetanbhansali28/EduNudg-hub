...
Each course card keeps **Enroll now** only (no separate **Enroll** price/link). The star rating sits below that button and is centered. The section has no **All courses** / course-name filter tabs. Regression: `regression_spark_courses_lists_all_published_programs`, `regression_spark_view_all_courses_hidden_when_catalog_fits`.
