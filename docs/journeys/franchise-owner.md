...
5. Franchise operator logs in on **center host** `/app` — Leads, Students, Fees, Inventory, and Merchandise share Curriculum pipeline chrome (header, KPI stats, search, list + detail) (no public franchise branding). Existing students can be bulk-enrolled from **Students** → **Import students** ([ops](../ops/center-student-csv-import.md)). **Copy Profile URL** on student Portal access shares the learn-portal login (no password) with parents.
