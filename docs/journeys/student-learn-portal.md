...
3. Student authenticates on the learn host; `/login` shows the franchise public header and footer when Student Login (or Copy Profile URL) included `?center=` (or the visitor came from the franchise site). `resolve_student_for_learn` auto-links when Auth email matches `login_email` and `user_id` is null (center **Linked** filter then shows 1).
