-- Franchise public landing shows only programs assigned to that center.

CREATE OR REPLACE FUNCTION public.center_public_curriculum_json(p_brand_id uuid, p_center_id uuid)
RETURNS jsonb
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT COALESCE(
    (
      ...
      FROM jsonb_array_elements(public.brand_public_curriculum_json(p_brand_id)) AS prog
      WHERE p_center_id IS NOT NULL
        AND lower(trim(prog ->> 'name')) IN (
          SELECT lower(trim(p.name))
          FROM public.center_program_enablement cpe
          JOIN public.programs p ON p.id = cpe.program_id
...
REVOKE ALL ON FUNCTION public.center_public_curriculum_json(uuid, uuid) FROM PUBLIC, anon, authenticated;
