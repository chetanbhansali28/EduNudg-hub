-- Center chrome + login use Franchise Identity display_name when set.
...
AS $$
  ...
  center_row AS (
    SELECT c.id, c.slug, c.name, c.display_name, c.brand_id
  ...
  SELECT COALESCE(
    jsonb_build_object(
      ...
      'brand_logo_url', COALESCE(
        NULLIF(trim(s.settings #>> '{landing,meta,logoUrl}'), ''),
        br.logo_url
      ),
      'center_id', cr.id,
      'center_slug', cr.slug,
      'center_name', COALESCE(NULLIF(trim(cr.display_name), ''), cr.name),
