-- Ensure ephemeral E2E brand purge also clears brand_subscriptions explicitly
-- (ON DELETE CASCADE already removes them; count + RPC path for E2E without DATABASE_URL).
-- Soft-archived E2E brands still appear on /admin/subscriptions until hard-deleted.
...
BEGIN
  ...
  IF cardinality(v_brand_ids) > 0 THEN
    ...
    DELETE FROM public.brand_subscriptions WHERE brand_id = ANY (v_brand_ids);
  ...
  RETURN jsonb_build_object(
    ...
    'brand_subscriptions_deleted', v_subs_deleted
  );
END;
$$;
