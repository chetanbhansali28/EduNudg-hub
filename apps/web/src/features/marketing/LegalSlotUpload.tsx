...
import { uploadPlatformLegalPage } from "@/lib/platformLegalStorage";
import {
...
export type LegalSlotUploadScope =
  | { mode: "brand"; brandId: string | null }
  | { mode: "platform" };

type Props = {
  scope: LegalSlotUploadScope;
...
export function LegalSlotUpload({ scope, kind, document, onUploaded, compact }: Props) {
  const inputId = useId();
  const inputRef = useRef<HTMLInputElement>(null);
  const [pending, setPending] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const canUpload = scope.mode === "platform" || Boolean(scope.brandId);
  ...
  const handleFile = async (file: File | undefined) => {
    ...
    try {
      const uploaded =
        scope.mode === "platform"
          ? await uploadPlatformLegalPage(kind, file)
          : await uploadBrandLegalPage(scope.brandId!, kind, file);
      onUploaded(uploaded);
    } catch (err) {
      ...
      {document ? (
        <div className="ed-brand-settings-legal-slot__current">
          ...
          <div className="ed-brand-settings-legal-slot__actions">
            <Button type="button" variant="secondary" disabled={!canUpload || pending} onClick={() => inputRef.current?.click()}>
